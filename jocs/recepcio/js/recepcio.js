...
tiles.map(tile => {
	tile.addEventListener("click", event => {
		...
		if(!cambio){
			tileArea1 = tile.style.getPropertyValue("--area");
			tile1=tile;
			tile1.style.opacity="0.5"
			cambio=true;
		}
		else{
			tileArea2 = tile.style.getPropertyValue("--area");
			cambio=false;
			tile1.style.opacity="1";
			tile1.style.setProperty("--area", tileArea2);
			tile.style.setProperty("--area", tileArea1);
		}
...
const isComplete = tiles => {
	...
	if (currentTilesString == Object.keys(areaKeys).toString()) {
		...
		if(temp[0].idcard-resultatAnterior==20){
			finalJuego();
		}
		setTimeout(() => {
			canvidePuzzle();
...
function finalJuego() {
	...
	temp[0].idcard = parseInt(document.getElementById("score").innerHTML);
